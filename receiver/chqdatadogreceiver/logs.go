...
func (ddr *datadogReceiver) splitLogs(logs []DDLog) []groupedLogs {
	cachedTags := newLocalTagCache()

	logkeys := make(map[int64]groupedLogs)
	for _, log := range logs {
		tags := splitTags(log.DDTags)
		for _, tag := range cachedTags.FetchCache(ddr.tagcacheExtension, log.Hostname) {

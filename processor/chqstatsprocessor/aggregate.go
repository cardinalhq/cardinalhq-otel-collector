...
func (e *beagle) emitSetI(set *ottl.AggregationSet[int64]) {
	now := time.Now()
...
func (e *beagle) emitSetF(set *ottl.AggregationSet[float64]) {
	now := time.Now()
	for _, agg := range set.Aggregations {
		mmetrics := pmetric.NewMetrics()
		res := mmetrics.ResourceMetrics().AppendEmpty()
...
func (e *beagle) aggregate(rms pmetric.ResourceMetrics, ils pmetric.ScopeMetrics, metric pmetric.Metric, dp pmetric.NumberDataPoint) bool {
	switch metric.Type() {
...
func (e *beagle) aggregateGaugeDatapoint(rms pmetric.ResourceMetrics, ils pmetric.ScopeMetrics, metric pmetric.Metric, dp pmetric.NumberDataPoint) bool {
...
func (e *beagle) aggregateSumDatapoint(rms pmetric.ResourceMetrics, ils pmetric.ScopeMetrics, metric pmetric.Metric, dp pmetric.NumberDataPoint) bool {
	metadata := map[string]string{
		"resource.schemaurl":            rms.SchemaUrl(),
		"instrumentation.schemaurl":     ils.SchemaUrl(),
		"instrumentation.name":          ils.Scope().Name(),
...
func (e *beagle) aggregateDatapoint(
